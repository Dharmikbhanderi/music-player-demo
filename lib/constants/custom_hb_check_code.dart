...
class CustomHBCheckCode extends StatefulWidget {
  final String? code;
  final int dotCount;
  final double width;
  final double height;
...
class _HBCheckCodeState extends State<CustomHBCheckCode> {
  //随机生成绘图数据
  Map getRandomData() {
    // 数字list
  ...
  Widget build(BuildContext context) {
    ...
    //计算最大宽度做自适应
    maxWidth = getTextSize(("8" * (widget.code?.length ?? 0)),
        TextStyle(
  ...
  Size getTextSize(String text, TextStyle style) {
    final TextPainter textPainter = TextPainter(
      ...
      ..layout(minWidth: 5, maxWidth: double.infinity);
